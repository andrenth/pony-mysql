...
class iso _TestInsert is UnitTest
  ...
  fun ref apply(h: TestHelper) ? =>
    ...
    let my_date: QueryParam    = Date(Time.now()._1)
    ...
    DB(h.env, query, params, lambda(res: Result)(h) =>
...
class iso _TestOption is UnitTest
  ...
  fun ref apply(h: TestHelper) ? =>
    ...
    DB(h.env, query, [tmout, charset], lambda(res: Result)(h) ? =>
...
class iso _TestNumResults is UnitTest
  ...
  fun ref apply(h: TestHelper) ? =>
    ...
    DB(h.env, query, Array[QueryParam](), lambda(res: Result)(h) =>
